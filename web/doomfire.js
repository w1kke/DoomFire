...
function createCanvasRenderer(canvas, settings = {}) {
  let simulation = createFireSimulation(settings);
...
function prepareCanvas(canvas, context, simulation) {
  ...
  return context.createImageData(simulation.width, simulation.height);
}
...
function renderFrame(simulation, target) {
  const { buffer, palette } = simulation;
  const frame = target || new Uint8ClampedArray(buffer.length * 4);
